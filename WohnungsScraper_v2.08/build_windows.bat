...
echo Version 5.07 - Mit Chrome Portable
echo ========================================
echo.
...
powershell -Command "& {[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%CHROME_URL%' -OutFile '%CHROME_INSTALLER%' -UseBasicParsing}"

if not exist "%CHROME_INSTALLER%" (
    echo [WARNUNG] Chrome Portable Download fehlgeschlagen!
    echo Versuche alternativen Download...
    
    REM Alternativer Download mit curl (falls vorhanden)
    curl -L -o "%CHROME_INSTALLER%" "%CHROME_URL%" 2>nul
)
...
echo.
echo [8/9] Kopiere Chrome Portable in dist...
if exist "%CHROME_DIR%" (
